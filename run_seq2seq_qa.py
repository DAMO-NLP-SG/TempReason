...
import transformers
from trainer_seq2seq_qa import QuestionAnsweringSeq2SeqTrainer, CustomCallback
...
class DataTrainingArguments:
    ...
    pretrain_dir: Optional[str] = field(
        default=None,
        metadata={"help": "An optional pretrained model"},
    )
    ...
    num_return_sequences: Optional[int] = field(
        ...
        metadata={
            "help": (
                "Number of sequences to return for evaluation. This argument will be passed to ``model.generate``, "
                "which is used during ``evaluate`` and ``predict``."
            )
        },
    )
...
def main():
    ...
    #splits = ['train', 'distant_train', 'val', 'test']  
    #train_splits = ['anti_train']
    if data_args.dlc == True:
        dlc_dir = './' 
    else:
        dlc_dir = './' 
    ...
    if data_args.pretrain_dir is not None:
        model.load_state_dict(torch.load(data_args.pretrain_dir + '/pytorch_model.bin'), strict=True)
    ...
    def preprocess_squad_batch(
        ...
        def generate_input(_question, _context):
            #return " ".join(["Please answer the following question: ", _question.lstrip(), _context.lstrip()])
            return " ".join([ _question.lstrip(), _context.lstrip()])
    ...
    def preprocess_function(examples):
        ...
        def generate_extractive_labels(_input_txt, _target_txt):
            return [1 if ((x in _target_txt) and (x not in  [tokenizer.pad_token, -100])) else 0 for x in _input_txt]
    ...
    def preprocess_validation_function(examples):
        ...
        model_inputs["labels"] = labels["input_ids"]
        #model_inputs["output_attention"] = True
    ...
    def post_processing_function(
        examples: datasets.Dataset, features: datasets.Dataset, outputs: EvalLoopOutput, stage="eval" ,num_return_sequences=1
    ):
    ...
    def post_processing_function_debug(
        examples: datasets.Dataset, features: datasets.Dataset, outputs: EvalLoopOutput, stage="eval" ,num_return_sequences=1
    ):
        # Decode the predicted tokens.
        preds = outputs.predictions
        ...
        if len(decoded_preds) == num_return_sequences * len(examples):

            grouped_preds = []
            for i in range(len(examples)):
                grouped_preds.append(decoded_preds[i*(num_return_sequences):i*(num_return_sequences) + num_return_sequences])
            # Filter out top predictions in beam searches (already decoded)
            # filter_preds = np.stack([x[0] for x in grouped_preds])
        # Build a map example to its corresponding features.
        example_id_to_index = {k: i for i, k in enumerate(examples["id"])}
        feature_per_example = {example_id_to_index[feature["example_id"]]: i for i, feature in enumerate(features)}
        predictions = {}
        ...
        for example_index, example in enumerate(examples):
            # This is the index of the feature associated to the current example.
            feature_index = feature_per_example[example_index]
            predictions[example["id"]] = grouped_preds[feature_index]

        # Format the result to the format the metric expects.
        if data_args.version_2_with_negative:
            formatted_predictions = [
                {"id": k, "prediction_text": v[0], "no_answer_probability": 0.0, "all_predictions": v } for k, v in predictions.items()
            ]
        else:
            formatted_predictions = [{"id": k, "prediction_text": v[0], "all_predictions": v } for k, v in predictions.items()]

        references = [{"id": ex["id"], "answers": ex[answer_column]} for ex in examples]
        
        eval_output = EvalPrediction(predictions=formatted_predictions, label_ids=references)
        return eval_output    
    ...
    if training_args.do_predict:
        logger.info("*** Predict ***")
        results = trainer.predict(predict_dataset, predict_examples, num_return_sequences=num_return_sequences)
